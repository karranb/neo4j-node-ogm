...
describe('Use Cases - 01', () => {
  ...
  describe('::create', () => {
    ...
    it('create toString', () => {
      ...
      expect(user.save().toString()).to.be.eql(`CREATE (user:User) SET user.email = $param1 , user.created_at = $param2 RETURN user {id:id(user), .name, .language, .email, .active, .password, .created_at }`)
    })
